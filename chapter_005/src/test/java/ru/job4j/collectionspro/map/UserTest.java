...
public class UserTest {
    ...
    public void whenCreateTwoDublicatsWithoutOverrideHashcode() {
         ...
         *
         * Что произойдет если переопределить только equals?
         * при определении условии уникальности(equals) и не определении hashcode.
         * хешкоды добовляемых обьектов (ключи) будут разными (разное значение ссылок обьектов)
         * видно из проверки условия if (p.hash == hash && ((k = p.key) == key || (key != null && key.equals(k))))
         * обьекты не будут считаться равными и произойдет вставка нового обьекта.
         *
         * Что произойдет если переопределить equals и hashcode?
         * Все условия иникальности определены
         * соответсвенного у двух одинаковых обьектов и ключи будут одинаковые
         * произойдет затирание старого обьекта. В Java нет возможности хранить мультизначение для ключа
         */
